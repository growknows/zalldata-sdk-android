...
import com.zalldata.analytics.android.sdk.exceptions.ConnectErrorException;
import com.zalldata.analytics.android.sdk.exceptions.DebugModeException;
import com.zalldata.analytics.android.sdk.exceptions.InvalidDataException;
import com.zalldata.analytics.android.sdk.exceptions.ResponseErrorException;
import com.zalldata.analytics.android.sdk.util.Base64Coder;
import com.zalldata.analytics.android.sdk.util.JSONUtils;
...
/**
 * Manage communication of events with the internal database and the ZallData servers.
 * This class straddles the thread boundary between user threads and
 * a logical ZallData thread.
 */
class AnalyticsMessages {
    private static final String TAG = "ZA.AnalyticsMessages";
    ...
    private ZallDataAPI mZallDataAPI;
    ...
    void enqueueEventMessage(final String type, final JSONObject eventJson) {
        try {
            synchronized (mDbAdapter) {
                ...
                if (mZallDataAPI.isDebugMode() || ret ==
                        DbParams.DB_OUT_OF_MEMORY_ERROR) {
                    mWorker.runMessage(m);
                } else {
                    // track_signup 立即发送
                    if (type.equals("track_signup") || ret > mZallDataAPI
                            .getFlushBulkSize()) {
                        mWorker.runMessage(m);
                    } else {
                        final int interval = mZallDataAPI.getFlushInterval();
                        mWorker.runMessageOnce(m, interval);
                    }
                }
            }
        } catch (Exception e) {
            ZALog.i(TAG, "enqueueEventMessage error:" + e);
        }
    }
    ...
    void flushScheduled() {
        try {
            ...
            mWorker.runMessageOnce(m, mZallDataAPI.getFlushInterval());
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    }
    ...
    private void sendData() {
        try {
            if (!mZallDataAPI.isNetworkRequestEnable()) {
                ZALog.i(TAG, "NetworkRequest 已关闭，不发送数据！");
                return;
            }

            if (TextUtils.isEmpty(mZallDataAPI.getServerUrl())) {
            ...
            if (!NetworkUtils.isShouldFlush(networkType, mZallDataAPI.getFlushNetworkPolicy())) {
                ZALog.i(TAG, String.format("您当前网络为 %s，无法发送数据，请确认您的网络发送策略！", networkType));
                return;
            }

            // 如果开启多进程上报
            if (mZallDataAPI.getConfigOptions().isMultiProcessFlush()) {
                // 已经有进程在上报
                if (DbAdapter.getInstance().isSubProcessFlushing()) {
                    return;
                }
                DbAdapter.getInstance().commitSubProcessFlushState(true);
            } else if (!ZallDataAPI.mIsMainProcess) {//不是主进程
                return;
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        ...
        while (count > 0) {
            ...
            try {
                ...
                if (!TextUtils.isEmpty(data)) {
                    sendHttpRequest(mZallDataAPI.getServerUrl(), data, gzip, rawMessage, false);
                }
                ...
                boolean isDebugMode = mZallDataAPI.isDebugMode();
                if (!TextUtils.isEmpty(errorMessage)) {
                    if (isDebugMode || ZALog.isLogEnabled()) {
                        ZALog.i(TAG, errorMessage);
                        if (isDebugMode && ZallDataAPI.SHOW_DEBUG_INFO_VIEW) {
                            try {
                                /*
                ...
                if (deleteEvents || isDebugMode) {
                    ...
                    ZALog.i(TAG, String.format(Locale.CHINA, "Events flushed. [left = %d]", count));
                } else {
                    count = 0;
                }
    ...
    private void sendHttpRequest(String path, String data, String gzip, String rawMessage, boolean isRedirects) throws ConnectErrorException, ResponseErrorException {
        ...
        try {
            ...
            if (connection == null) {
                ZALog.i(TAG, String.format("can not connect %s, it shouldn't happen", url.toString()), null);
                return;
            }
            ...
            if (mZallDataAPI.getDebugMode() == ZallDataAPI.DebugMode.DEBUG_ONLY) {
                connection.addRequestProperty("Dry-Run", "true");
            }

            String cookie = mZallDataAPI.getCookie(false);
            ...
            if (ZALog.isLogEnabled()) {
                ...
                if (responseCode >= HttpURLConnection.HTTP_OK &&
                    ...
                    ZALog.i(TAG, "valid message: \n" + jsonMessage);
                } else {
                    ZALog.i(TAG, "invalid message: \n" + jsonMessage);
                    ZALog.i(TAG, String.format(Locale.CHINA, "ret_code: %d", responseCode));
                    ZALog.i(TAG, String.format(Locale.CHINA, "ret_content: %s", response));
                }
            }
    ...
    private class Worker {
        ...
        Worker() {
            final HandlerThread thread =
                    new HandlerThread("com.zalldata.analytics.android.sdk.AnalyticsMessages.Worker",
                            Thread.MIN_PRIORITY);
            thread.start();
        ...
        void runMessage(Message msg) {
            synchronized (mHandlerLock) {
                ...
                if (mHandler == null) {
                    ZALog.i(TAG, "Dead worker dropping a message: " + msg.what);
                } else {
            ...
            public void handleMessage(Message msg) {
                try {
                    if (msg.what == FLUSH_QUEUE) {
                        ...
                        ZALog.i(TAG, "Unexpected message received by ZallData worker: " + msg);
                    }
                } catch (final RuntimeException e) {
                    ZALog.i(TAG, "Worker threw an unhandled exception", e);
                }
            }
        }
    }
