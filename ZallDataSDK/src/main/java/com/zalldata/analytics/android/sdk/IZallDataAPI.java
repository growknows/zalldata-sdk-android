...
public interface IZallDataAPI extends IFragmentAPI {
    /**
     * 返回预置属性
     *
     ...
     * https://zalldata.cn/manual/android_sdk.html
     * 该功能仅在 API 14 及以上版本中生效，默认关闭
     *
     * @param eventTypeList 开启 AutoTrack 的事件列表
     */
    void enableAutoTrack(List<ZallDataAPI.AutoTrackEventType> eventTypeList);
     ...
     */
    void disableAutoTrack(List<ZallDataAPI.AutoTrackEventType> eventTypeList);
     ...
     */
    void disableAutoTrack(ZallDataAPI.AutoTrackEventType autoTrackEventType);
     ...
     */
    boolean isAutoTrackEventTypeIgnored(ZallDataAPI.AutoTrackEventType eventType);
    ...
    /**
     * 获取当前用户的匿名 ID
     * 若调用前未调用 {@link #identify(String)} 设置用户的匿名 ID，SDK 会优先调用 {@link com.zalldata.analytics.android.sdk.util.ZallDataUtils#getAndroidID(Context)}获取 Android ID，
     ...
     */
    void registerDynamicSuperProperties(ZallDataDynamicSuperProperties dynamicSuperProperties);
     ...
     */
    void setTrackEventCallBack(ZallDataTrackEventCallBack trackEventCallBack);
     ...
     */
    void setDeepLinkCallback(ZallDataDeepLinkCallback deepLinkCallback);
     ...
     * @param latitude 纬度
     * @param longitude 经度
     * @param coordinate 坐标系，坐标系类型请参照 {@link ZallDataGPSLocation.CoordinateType}
     */
