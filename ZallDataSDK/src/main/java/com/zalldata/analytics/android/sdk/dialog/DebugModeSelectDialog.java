...
import com.zalldata.analytics.android.sdk.util.ZADisplayUtil;
...
class DebugModeSelectDialog extends Dialog implements View.OnClickListener {
    ...
    private ZallDataAPI.DebugMode currentDebugMode;

    DebugModeSelectDialog(Context context, ZallDataAPI.DebugMode debugMode) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (window != null) {
            ...
            p.width = ZADisplayUtil.dip2px(getContext(), 270);
            p.height = ZADisplayUtil.dip2px(getContext(), 240);
            window.setAttributes(p);
            //设置弹框圆角
            GradientDrawable bg = new GradientDrawable();
            bg.setShape(GradientDrawable.RECTANGLE);
            bg.setColor(Color.WHITE);
            bg.setCornerRadius(ZADisplayUtil.dip2px(getContext(), 7));
    ...
    private void initView() {
        ...
        String msg = "调试模式已关闭";
        if (currentDebugMode == ZallDataAPI.DebugMode.DEBUG_ONLY) {
            msg = "当前为 调试模式（不导入数据）";
    ...
    interface OnDebugModeViewClickListener {
        ...
        void setDebugMode(Dialog dialog, ZallDataAPI.DebugMode debugMode);
    }
}
