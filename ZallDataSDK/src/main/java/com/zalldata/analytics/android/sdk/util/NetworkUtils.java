...
public class NetworkUtils {
    ...
    public static String networkType(Context context) {
        try {
            ...
            // 检测权限
            if (!ZallDataUtils.checkHasPermission(context, Manifest.permission.ACCESS_NETWORK_STATE)) {
                networkType = "NULL";
                return networkType;
            }
    ...
    private static int toNetworkType(String networkType) {
        if ("NULL".equals(networkType)) {
            return ZallDataAPI.NetworkType.TYPE_ALL;
        } else if ("WIFI".equals(networkType)) {
            return ZallDataAPI.NetworkType.TYPE_WIFI;
        } else if ("2G".equals(networkType)) {
            return ZallDataAPI.NetworkType.TYPE_2G;
        } else if ("3G".equals(networkType)) {
            return ZallDataAPI.NetworkType.TYPE_3G;
        } else if ("4G".equals(networkType)) {
            return ZallDataAPI.NetworkType.TYPE_4G;
        } else if ("5G".equals(networkType)) {
            return ZallDataAPI.NetworkType.TYPE_5G;
        }
        return ZallDataAPI.NetworkType.TYPE_ALL;
    }
    ...
    public static void registerNetworkListener(Context context) {
        try {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                ZALog.i(TAG, "Register BroadcastReceiver");
            } else {
                if (networkCallback == null) {
                    networkCallback = new ZANetworkCallbackImpl();
                }
                ...
                if (connectivityManager != null) {
                    ...
                    ZALog.i(TAG, "Register ConnectivityManager");
                }
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
    ...
    public static void unregisterNetworkListener(Context context) {
        try {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                ZALog.i(TAG, "unregisterReceiver BroadcastReceiver");
            } else {
                if (networkCallback == null) {
                    return;
                }
                ...
                if (connectivityManager != null) {
                    ...
                    ZALog.i(TAG, "unregister ConnectivityManager");
                }
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
    ...
    private static String mobileNetworkType(Context context, TelephonyManager telephonyManager, ConnectivityManager connectivityManager) {
        ...
        if (telephonyManager != null) {
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.R
                    && (ZallDataUtils.checkHasPermission(context, Manifest.permission.READ_PHONE_STATE) || telephonyManager.hasCarrierPrivileges())) {
    ...
    private static class ZANetworkCallbackImpl extends ConnectivityManager.NetworkCallback {
        ...
        public void onAvailable(Network network) {
            ...
            NetworkUtils.cleanNetworkTypeCache();
            ZallDataAPI.sharedInstance().flush();
            ZALog.i(TAG, "onAvailable is calling");
        }
        ...
        public void onCapabilitiesChanged(Network network, NetworkCapabilities networkCapabilities) {
            ...
            NetworkUtils.cleanNetworkTypeCache();
            ZALog.i(TAG, "onCapabilitiesChanged is calling");
        }
