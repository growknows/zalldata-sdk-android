...
package com.zalldata.analytics.android.sdk.dialog;

import static com.zalldata.analytics.android.sdk.util.Base64Coder.CHARSET_UTF8;
...
import com.zalldata.analytics.android.sdk.ThreadNameConstants;
...
import com.zalldata.analytics.android.sdk.util.NetworkUtils;
...
import com.zalldata.analytics.android.sdk.visual.view.PairingCodeEditDialog;
...
public class ZallDataDialogUtils {
    private static final String TAG = "ZA.ZallDataDialogUtils";
    ...
    public static void showChannelDebugDialog(final Activity activity,
        ...
        ZallDataDialogUtils.showDialog(activity, "即将开启联调模式", "", "确定", new DialogInterface.OnClickListener() {
            @Override
                    ...
                    requestActiveChannel(baseUrl,
                            ...
                            new HttpCallback.JsonCallback() {
                                ...
                                public void onFailure(int code, String errorMessage) {
                                    ...
                                    ZALog.i(TAG, "ChannelDebug request error:" + errorMessage);
                                ...
                                public void onResponse(JSONObject response) {
                                    loadingDialog.dismiss();
                                    if (response == null) {
                                        ZALog.i(TAG, "ChannelDebug response error msg: response is null");
                                    ...
                                    if (code == 1) {// 请求成功
                                        ...
                                        ZALog.i(TAG, "ChannelDebug response error msg:" + response.optString("message"));
                                        showDialog(activity, "添加白名单请求失败，请联系卓尔技术支持人员排查问题!");
                                    }
                                }
                            });
    ...
    private static void showChannelDebugErrorDialog(final Activity activity) {
        ZallDataDialogUtils.showDialog(activity, "检测到 “设备码为空”，可能原因如下，请排查：",
                "1. 开启 App 时拒绝“电话”授权；\n" +
    ...
    public static void showPopupWindowDialog(final Activity activity, Uri uri) {
        try {
            Class<?> clazz = Class.forName("com.zalldata.sf.ui.utils.PreviewUtil");
    ...
    public static void showDebugModeSelectDialog(final Activity activity, final String infoId, final String locationHref, final String project) {
        try {
            DebugModeSelectDialog dialog = new DebugModeSelectDialog(activity, ZallDataAPI.sharedInstance().getDebugMode());
            ...
            dialog.setOnDebugModeDialogClickListener(new DebugModeSelectDialog.OnDebugModeViewClickListener() {
                ...
                @Override
                public void setDebugMode(Dialog dialog, ZallDataAPI.DebugMode debugMode) {
                    ZallDataAPI.sharedInstance().setDebugMode(debugMode);
                    dialog.cancel();
                }
            });
            dialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
                ...
                public void onCancel(DialogInterface dialog) {
                    ...
                    ZallDataAPI.DebugMode mCurrentDebugMode = ZallDataAPI.sharedInstance().getDebugMode();
                    if (ZallDataAPI.sharedInstance().isNetworkRequestEnable() && !TextUtils.isEmpty(serverUrl) && !TextUtils.isEmpty(infoId) && mCurrentDebugMode != ZallDataAPI.DebugMode.DEBUG_OFF) {
                        if (TextUtils.isEmpty(locationHref)) {
                            new SendDebugIdThread(serverUrl, ZallDataAPI.sharedInstance().getDistinctId(), infoId, ThreadNameConstants.THREAD_SEND_DISTINCT_ID).start();
                        } else {
                            try {
                                if (!TextUtils.isEmpty(project)) {
                                    String url = locationHref + "?project=" + project;
                                    ZALog.i(TAG, "sf url:" + url);
                                    new SendDebugIdThread(url, ZallDataAPI.sharedInstance().getDistinctId(), infoId, ThreadNameConstants.THREAD_SEND_DISTINCT_ID).start();
                                }
                            } catch (Exception e) {
                                ZALog.printStackTrace(e);
                            }
                    ...
                    if (mCurrentDebugMode == ZallDataAPI.DebugMode.DEBUG_OFF) {
                        currentDebugToastMsg = "已关闭调试模式，请重新扫描二维码进行开启";
                    } else if (mCurrentDebugMode == ZallDataAPI.DebugMode.DEBUG_ONLY) {
                        currentDebugToastMsg = "开启调试模式，校验数据，但不进行数据导入；关闭 App 进程后，将自动关闭调试模式";
                    } else if (mCurrentDebugMode == ZallDataAPI.DebugMode.DEBUG_AND_TRACK) {
                    ...
                    ZALog.info(TAG, "您当前的调试模式是：" + mCurrentDebugMode, null);
                    startLaunchActivity(activity);
                }
            });
            dialogShowDismissOld(dialog);
    ...
    public static void showOpenHeatMapDialog(final Activity context, final String featureCode,
        ...
        try {
            if (!ZallDataAPI.sharedInstance().isNetworkRequestEnable()) {
                showDialog(context, "已关闭网络请求（NetworkRequest），无法使用 App 点击分析，请开启后再试！");
                return;
            }

            if (!ZallDataAPI.sharedInstance().isHeatMapEnabled()) {
    ...
    public static void showOpenVisualizedAutoTrackDialog(final Activity context, final String featureCode, final String postUrl) {
        try {
            ...
            if (!ZallDataAPI.sharedInstance().isVisualizedAutoTrackEnabled()) {
    ...
    public static void showPairingCodeInputDialog(final Context context) {
        if (context == null) {
            ZALog.i(TAG, "The argument context can't be null");
            return;
        }
        if (!(context instanceof Activity)) {
            ZALog.i(TAG, "The static method showPairingCodeEditDialog(Context context) only accepts Activity as a parameter");
            return;
        }
    ...
    public static void startLaunchActivity(Context context) {
        try {
            if (isSchemeActivity(context)) {
                ...
                ZALog.i(TAG, "startLaunchActivity");
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    ...
    private static void requestActiveChannel(String baseUrl, String monitorId,
        ...
        try {
            ...
            json.put("distinct_id", ZallDataAPI.sharedInstance().getDistinctId());
    ...
    private static class SendDebugIdThread extends Thread {
        ...
        private void sendHttpRequest(String serverUrl, boolean isRedirects) {
            ...
            try {
                ...
                ZALog.info(TAG, String.format("DebugMode URL:%s", url), null);
                connection = (HttpURLConnection) url.openConnection();
                if (connection == null) {
                    ZALog.info(TAG, String.format("can not connect %s,shouldn't happen", url.toString()), null);
                    return;
                }
                ...
                writer.flush();
                ZALog.info(TAG, String.format("DebugMode request body : %s", requestBody), null);
                ...
                ZALog.info(TAG, String.format(Locale.CHINA, "DebugMode 后端的响应码是:%d", responseCode), null);
