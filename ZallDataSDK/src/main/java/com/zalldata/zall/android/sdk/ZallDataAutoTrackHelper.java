...
import com.zalldata.zall.android.sdk.util.ReflectUtil;
import com.zalldata.zall.android.sdk.util.ZAFragmentUtils;
import com.zalldata.zall.android.sdk.util.ZallDataUtils;
import com.zalldata.zall.android.sdk.util.ThreadUtils;
import com.zalldata.zall.android.sdk.util.ViewUtil;
import com.zalldata.zall.android.sdk.util.WindowHelper;
import com.zalldata.zall.android.sdk.visual.WebViewVisualInterface;
import com.zalldata.zall.android.sdk.visual.model.ViewNode;
import com.zalldata.zall.android.sdk.visual.util.VisualUtil;
...
public class ZallDataAutoTrackHelper {
    private static final String TAG = "ZallDataAutoTrackHelper";
    ...
    public static void trackExpandableListViewOnGroupClick(ExpandableListView expandableListView, View view,
        ...
        try {
            ...
            //关闭 AutoTrack
            if (!ZallDataAPI.sharedInstance().isAutoTrackEnabled()) {
                return;
            }
            ...
            //Activity 被忽略
            if (activity != null) {
                if (ZallDataAPI.sharedInstance().isActivityAutoTrackAppClickIgnored(activity.getClass())) {
                    return;
                }
            }
            ...
            // fragment 忽略
            if (fragment != null) {
                if (ZallDataAPI.sharedInstance().isActivityAutoTrackAppClickIgnored(fragment.getClass())) {
                    return;
                }
            }
            ...
            // $screen_name & $title
            if (activity != null) {
                ZallDataUtils.mergeJSONObject(AopUtil.buildTitleAndScreenName(activity), properties);
            }
            ...
            ZallDataAPI.sharedInstance().trackAutoEvent(AopConstants.APP_CLICK_EVENT_NAME, properties, viewNode);
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    }
    ...
    public static void trackTabLayoutSelected(Object object, Object tab) {
        try {
                ...
                try {
                    ...
                    for (Field field : fields) {
                        ...
                        if (bridgeObject instanceof Activity) {
                            ...
                            break;
                        } else if (ZAFragmentUtils.isFragment(bridgeObject)) {
                            object = bridgeObject;
                            isFragment = true;
                            break;
            ...
            if (isFragment) {
                if (ZallDataAPI.sharedInstance().isActivityAutoTrackAppClickIgnored(object.getClass())) {
                    return;
                }
            }
    ...
    public static void trackDrawerOpened(View view) {
        try {
            ...
            ZallDataAPI.sharedInstance().setViewProperties(view, jsonObject);
    ...
    public static void trackViewOnClick(View view, boolean isFromUser) {
        try {
            ...
            if (ZallDataUtils.isDoubleClick(view)) {
                return;
            }
            ...
            if (AopUtil.injectClickInfo(view, properties, isFromUser)) {
                ZallDataAPI.sharedInstance().trackAutoEvent(AopConstants.APP_CLICK_EVENT_NAME, properties, AopUtil.addViewPathProperties(activity, view, properties));
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    ...
    public static void track(String eventName, String properties) {
        try {
            ...
            ZallDataAPI.sharedInstance().trackInternal(eventName, pro);
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    }
    ...
    public static void showChannelDebugActiveDialog(final Activity activity) {
        ZallDataDialogUtils.showDialog(activity, "成功开启调试模式",
                "此模式下不需要卸载 App，点击“激活”按钮可反复触发激活", "激活", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        ZallDataAPI.sharedInstance().trackChannelDebugInstallation();
    ...
    public static void loadUrl2(View webView, String url) {
        if (webView == null) {
            ZALog.i(TAG, "WebView has not initialized.");
            return;
        }
    ...
    private static void setupH5Bridge(View webView) {
        ...
        if (isSupportJellyBean() && ZallDataAPI.sharedInstance().getConfigOptions() != null && ZallDataAPI.sharedInstance().getConfigOptions().isAutoTrackWebView) {
    ...
    private static boolean isSupportJellyBean() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1 && !ZallDataAPI.getConfigOptions().isWebViewSupportJellyBean) {
