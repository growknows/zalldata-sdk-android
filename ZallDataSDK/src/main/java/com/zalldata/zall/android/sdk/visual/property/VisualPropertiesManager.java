...
package com.zalldata.zall.android.sdk.visual.property;
...
import com.zalldata.zall.android.sdk.util.ViewUtil;
import com.zalldata.zall.android.sdk.visual.ViewTreeStatusObservable;
import com.zalldata.zall.android.sdk.visual.model.ViewNode;
import com.zalldata.zall.android.sdk.visual.model.VisualConfig;
...
public class VisualPropertiesManager {
    ...
    private static final String TAG = "ZA.VP.VisualPropertiesManager";
    ...
    public void requestVisualConfig() {
        try {
            ...
            if (context != null) {
                requestVisualConfig(context, ZallDataAPI.sharedInstance());
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    ...
    public void mergeVisualProperties(VisualEventType eventType, JSONObject srcObject, ViewNode viewNode) {
        try {
            ...
            ZALog.i(TAG, String.format("mergeVisualProperties eventType: %s, screenName:%s ", eventType.getVisualEventType(), screenName));
            if (TextUtils.isEmpty(screenName)) {
                ZALog.i(TAG, "screenName is empty and return");
                return;
            }

            // 校验开关是否打开
            if (!ZallDataAPI.sharedInstance().isVisualizedAutoTrackEnabled()) {
                ZALog.i(TAG, "you should call 'enableVisualizedAutoTrack(true)' first");
                if (mCollectLogListener != null) {
            ...
            if (!(activity != null && ZallDataAPI.sharedInstance().isVisualizedAutoTrackActivity(activity.getClass()))) {
                ZALog.i(TAG, "activity is null or not in white list and return");
                if (mCollectLogListener != null) {
            ...
            // 缓存中不存在配置
            if (mVisualConfig == null) {
                ZALog.i(TAG, "visual properties is empty and return");
                if (mCollectLogListener != null) {
            ...
            if (propertiesConfigs == null || propertiesConfigs.size() == 0) {
                ZALog.i(TAG, "propertiesConfigs is empty");
                if (mCollectLogListener != null) {
            ...
            if (eventConfigList.size() == 0) {
                ZALog.i(TAG, "event config is empty and return");
                if (mCollectLogListener != null) {
            ...
            for (VisualConfig.VisualPropertiesConfig visualPropertiesConfig : eventConfigList) {
                ...
                if (properties == null || properties.size() == 0) {
                    ZALog.i(TAG, "properties is empty ");
                    continue;
                }
            ...
            for (VisualConfig.VisualPropertiesConfig visualPropertiesConfig : propertiesConfigs) {
                ...
                if (eventType == VisualEventType.APP_CLICK || eventType == VisualEventType.WEB_CLICK) {
                    ...
                    if (!TextUtils.equals(event.elementPath, elementPath)) {
                        ZALog.i(TAG, String.format("event element_path is not match: current element_path is %s, config element_path is %s ", elementPath, event.elementPath));
                        continue;
                    }

                    // 如果限定了位置，则必须校验 elementPosition
                    if (event.limitElementPosition && !TextUtils.equals(event.elementPosition, elementPosition)) {
                        ZALog.i(TAG, String.format("event element_position is not match: current element_position is %s, config element_position is %s ", elementPosition, event.elementPosition));
                        continue;
                    }

                    // 如果限定了内容，则必须校验 elementContent
                    if (event.limitElementContent && !TextUtils.equals(event.elementContent, elementContent)) {
                        ZALog.i(TAG, String.format("event element_content is not match: current element_content is %s, config element_content is %s ", elementContent, event.elementContent));
                        continue;
                    }
                }
    ...
    public boolean checkAppIdAndProject() {
        ...
        if (TextUtils.isEmpty(serverUrl)) {
            ZALog.i(TAG, "serverUrl is empty and return");
            return false;
        }
        ...
        if (TextUtils.isEmpty(project) || TextUtils.isEmpty(appId)) {
            ZALog.i(TAG, "project or app_id is empty and return");
            return false;
        }

        if (mVisualConfig == null) {
            ZALog.i(TAG, "VisualConfig is null and return");
            return false;
        }
        // 校验配置中的 app_id 和 当前 app_id 是否一致
        if (!TextUtils.equals(appId, mVisualConfig.appId)) {
            ZALog.i(TAG, String.format("app_id is not equals: current app_id is %s, config app_id is %s ", appId, mVisualConfig.appId));
            return false;
        }

        // 校验配置中的 project 和 当前 project 是否一致
        if (!TextUtils.equals(project, mVisualConfig.project)) {
            ZALog.i(TAG, String.format("project is not equals: current project is %s, config project is %s ", project, mVisualConfig.project));
            return false;
        }
        return true;
    }
    ...
    public void mergeAppVisualProperty(VisualConfig.VisualProperty visualProperty, VisualConfig.VisualEvent event, JSONObject srcObject, ViewNode clickViewNode) {
        try {
            ...
            if (TextUtils.isEmpty(visualProperty.name)) {
                ZALog.i(TAG, "config visual property name is empty");
                return;
            }

            // 属性 element_path 非法校验
            if (TextUtils.isEmpty(visualProperty.elementPath)) {
                ZALog.i(TAG, "config visual property elementPath is empty");
                return;
            }
            ...
            if (clickViewNode != null && !TextUtils.isEmpty(clickViewNode.getViewPosition()) && event != null && !TextUtils.isEmpty(event.elementPosition) && !event.limitElementPosition && !TextUtils.isEmpty(visualProperty.elementPosition)) {
                if (TextUtils.equals(visualProperty.elementPath.split("-")[0], event.elementPath.split("-")[0])) {
                    ...
                    ZALog.i(TAG, "visualProperty elementPosition replace: " + clickViewNode.getViewPosition());
                }
            }
            ...
            ZALog.i(TAG, String.format("find property target view success, property element_path: %s,element_position: %s,element_content: %s", visualProperty.elementPath, visualProperty.elementPosition, propertyElementContent));
            // 开始正则处理
            String result = null;
            if (!TextUtils.isEmpty(visualProperty.regular)) {
                ...
                try {
                    ...
                    if (matcher.find()) {
                        ...
                        ZALog.i(TAG, String.format("propertyValue is: %s", result));
                    } else {
                        ZALog.i(TAG, "matcher not find continue");
                        if (mCollectLogListener != null) {
