...
public class VisualDebugHelper {
    ...
    private static final String TAG = "ZA.VP.VisualDebugHelper";
    ...
    void startMonitor() {
        try {
            ...
            ZallDataAPI.sharedInstance().addEventListener(mEventListener);
        } catch (Exception e) {
    ...
    void stopMonitor() {
        try {
            if (mEventListener != null) {
                ZallDataAPI.sharedInstance().removeEventListener(mEventListener);
            }
        } catch (Exception e) {
            ZALog.printStackTrace(e);
        }
    ...
    private synchronized void handlerEvent(JSONObject jsonObject) {
        try {
            ...
            if (!TextUtils.equals(AopConstants.APP_CLICK_EVENT_NAME, eventName) && !TextUtils.equals(AopConstants.WEB_CLICK_EVENT_NAME, eventName)) {
                ZALog.i(TAG, "eventName is " + eventName + " filter");
                return;
            }
            ...
            if (TextUtils.equals(AopConstants.APP_CLICK_EVENT_NAME, eventName)) {
                ...
                if (TextUtils.isEmpty(screenName)) {
                    ZALog.i(TAG, "screenName is empty ");
                    return;
                }
                    ...
                    synchronized (object) {
                        for (VisualConfig.VisualPropertiesConfig config : eventConfigList) {
                            try {
                                ...
                                ZallDataUtils.mergeJSONObject(jsonObject, object);
                ...
                try {
                    JSONArray array = propertyObject.optJSONArray("zalldata_web_visual_eventName");
                    if (array == null) {
    ...
    private abstract static class TrackEventAdapter implements ZAEventListener {
        @Override
        public void login() {
