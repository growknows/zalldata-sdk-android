...
package com.zalldata.zall.android.sdk.visual.view;
...
public class PairingCodeEditDialog extends Dialog {
    ...
    private static final String TAG = "ZA.PairingCodeEditDialog";
        ...
        pairingCodeEditText.setOnPairingCodeChangedListener(new IPairingCodeInterface.OnPairingCodeChangedListener() {
            ...
            public void onInputCompleted(CharSequence s) {
                if (TextUtils.isEmpty(s)) {
                    ZALog.i(TAG, "onCreate | dialog input content is null and return");
                    return;
                }
    ...
    public void show() {
        if (!this.isLiving()) {
            ZALog.i(TAG, "Activity is finish");
        } else {
            ZALog.i(TAG, "show:" + this.mContext);
            super.show();
        }
    }
    ...
    public void dismiss() {
        if (Looper.myLooper() != Looper.getMainLooper() && this.mContext instanceof Activity && isActivityFinishingOrDestroyed(this.mContext)) {
            ...
            if (this.isShowing()) {
                try {
                    ZALog.i(TAG, "isShowing() == true, dismiss");
                    super.dismiss();
    ...
    private boolean isActivityFinishingOrDestroyed(Context context) {
        ...
        if (activity.isFinishing()) {
            ZALog.i(TAG, "Activity is finish,name=" + activity.getClass().getName());
            return true;
        }
