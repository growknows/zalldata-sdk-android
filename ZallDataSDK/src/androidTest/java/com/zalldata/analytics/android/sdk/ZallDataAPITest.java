...
public class ZallDataAPITest {

    private static final String TAG = "ZallDataAPITest";
    ...
    public void eventPause() {
        Context context = ApplicationProvider.getApplicationContext();
        ZallDataAPI.startWithConfigOptions(context, new ZAConfigOptions(""));
        ZallDataAPI.sharedInstance().trackTimerStart("event_pause");
        Field field;
        Map<String, EventTimer> map;
        try {
            Thread.sleep(1000);
            field = ZallDataAPI.class.getDeclaredField("mTrackTimer");
            assertNotNull(field);
            field.setAccessible(true);
            map = (Map<String, EventTimer>) field.get(ZallDataAPI.sharedInstance());
            assertNotNull(map);
            assertEquals(1, map.size());
            Thread.sleep(9000);
            ZallDataAPI.sharedInstance().trackTimerPause("event_pause");
            Thread.sleep(10000);
            ZallDataAPI.sharedInstance().trackTimerResume("event_pause");
            Thread.sleep(10000);
    ...
    public void getLoginId() {
        try {
            ...
            Thread.sleep(1000);
            String loginId = ZallDataAPI.sharedInstance().getLoginId();
            // loginId 未登录时为空
            assertNull(loginId);
            // 登录
            ...
            // 先清除数据，不然一直会存在 xml 中
            ZallDataAPI.sharedInstance().login(null);
            loginId = ZallDataAPI.sharedInstance().getLoginId();
            assertEquals(loginId, userId);
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public void getDistinctId() {
        try {
            ...
            String distinct_id = ZallDataAPI.sharedInstance().getDistinctId();
            ...
            Thread.sleep(1000);
            loginId = ZallDataAPI.sharedInstance().getLoginId();
            distinct_id = ZallDataAPI.sharedInstance().getDistinctId();
            assertEquals(loginId, userId);
    ...
    public void itemSet() {
        try {
            Context context = ApplicationProvider.getApplicationContext();
            ZallDataAPI.startWithConfigOptions(context, new ZAConfigOptions(""));
            ZallDataAPI.sharedInstance().deleteAll();
            Thread.sleep(1000);
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("ProductType", "TV");
            jsonObject.put("ProductDetail", "watch Tv");
            ZallDataAPI.sharedInstance().itemSet("ProductSet", "100", jsonObject);
            // 刷新入库
            ZallDataAPI.sharedInstance().flush();
            Thread.sleep(1000);
            ContentResolver contentResolver = context.getContentResolver();
            Uri uri = Uri.parse("content://" + context.getPackageName() + ".ZallDataContentProvider/events");
    ...
    public void itemDelete() {
        try {
            ...
            ZallDataAPI.sharedInstance().deleteAll();
            Thread.sleep(1000);
            ZallDataAPI.sharedInstance().itemDelete("ProductDelete", "100");
            // 刷新入库
            ZallDataAPI.sharedInstance().flush();
            Thread.sleep(1000);
            ContentResolver contentResolver = context.getContentResolver();
    ...
    public void trackWithTimeProperty() {
        try {
            ...
            ZallDataAPI.sharedInstance().track("InvalidTime", jsonObject);
            ZallDataAPI.sharedInstance().flush();
            Thread.sleep(1000);
            ContentResolver contentResolver = context.getContentResolver();
            Cursor cursor = contentResolver.query(Uri.parse("content://" + context.getPackageName() + ".ZallDataContentProvider/events"), null, null, null);
            assertNotNull(cursor);
            ...
            ZallDataAPI.sharedInstance().flush();
            Thread.sleep(1000);
            cursor = contentResolver.query(Uri.parse("content://" + context.getPackageName() + ".ZallDataContentProvider/events"), null, null, null);
            assertNotNull(cursor);
    ...
    @Test
    public void ZAConfigOptionsTest() {
        try {
            ...
            assertEquals(MAX_CACHE_SIZE, ZallDataAPI.sharedInstance().getMaxCacheSize());
            assertEquals(NETWORK_TYPE, ZallDataAPI.sharedInstance().getFlushNetworkPolicy());

            assertEquals(FLAG, ZallDataAPI.sharedInstance().isVisualizedAutoTrackEnabled());
            assertEquals(FLAG, ZallDataAPI.sharedInstance().isHeatMapEnabled());
    ...
    public void maxCacheSizeTest() {
        ...
        ZallDataAPI.sharedInstance().setMaxCacheSize(greater);
        assertEquals(greater, ZallDataAPI.sharedInstance().getMaxCacheSize());
        long lesser = 10 * 1024 * 1024;
        ZallDataAPI.sharedInstance().setMaxCacheSize(lesser);
        assertEquals(normal, ZallDataAPI.sharedInstance().getMaxCacheSize());
        long negative = -1;
        ZallDataAPI.sharedInstance().setMaxCacheSize(negative);
        assertEquals(normal, ZallDataAPI.sharedInstance().getMaxCacheSize());
    }
}