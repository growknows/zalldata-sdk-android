    ...
    sd.initPara = function(para) {
      ...
      function initAppH5Status() {
        ...
        if (_.isObject(sd.para.app_js_bridge)) {
          if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.zalldataNativeTracker && _.isObject(window.ZallData_iOS_JS_Bridge) && window.ZallData_iOS_JS_Bridge.zalldata_app_server_url) {
            if (checkProjectAndHost(window.ZallData_iOS_JS_Bridge.zalldata_app_server_url)) {
              sd.para.app_js_bridge.H5verify = true;
            }else{
              sd.para.app_js_bridge.defineModeDebugInfo = '4'
            }

          } else if (_.isObject(window.ZallData_APP_New_H5_Bridge) && window.ZallData_APP_New_H5_Bridge.zalldata_get_server_url && window.ZallData_APP_New_H5_Bridge.zalldata_track) {
            var app_server_url = window.ZallData_APP_New_H5_Bridge.zalldata_get_server_url();
            if (app_server_url) {
      ...
      defineMode: function(type){
        var debugList = {
          '1': {
                  ...
                  "link_text": "配置文档",
                  "link_url": "https://manual.zalldata.cn/sa/latest/app-h5-1573913.html"
                },
          '2': {
                  "title": "当前页面无法进行可视化全埋点",
    ...
    var commonWays = {
      ...
      trackHeatMap: function(target, props, callback) {
        if ((typeof target === 'object') && target.tagName) {
          ...
          if (tagName !== 'button' && tagName !== 'a' && parent_ele !== 'a' && parent_ele !== 'button' && tagName !== 'input' && tagName !== 'textarea' && !_.hasAttribute(target, 'data-zall-click')) {
      ...
      setPlugin: function(para) {
        ...
        _.each(para, function(v, k) {
          if (_.isFunction(v)) {
            if (_.isObject(window.ZallDataWebJSSDKPlugin) && window.ZallDataWebJSSDKPlugin[k]) {
              v(window.ZallDataWebJSSDKPlugin[k]);
            } else {
      ...
      if (_.isObject(sd.para.app_js_bridge) || sd.para.app_js_bridge === 'only') {
        if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.zalldataNativeTracker && _.isObject(window.ZallData_iOS_JS_Bridge) && window.ZallData_iOS_JS_Bridge.zalldata_app_server_url) {
          if (sd.para.app_js_bridge.H5verify) {
            ...
            window.webkit.messageHandlers.zalldataNativeTracker.postMessage(JSON.stringify({
          ...
          if (sd.para.app_js_bridge.H5verify) {
            ...
            ZallData_APP_New_H5_Bridge.zalldata_track(JSON.stringify(_.extend({
              server_url: sd.para.server_url
            }, originData)));
          } else {
            if (sd.para.app_js_bridge.is_send) {
        ...
        } else if ((typeof ZallData_APP_JS_Bridge === 'object') && (ZallData_APP_JS_Bridge.zalldata_verify || ZallData_APP_JS_Bridge.zalldata_track)) {
          if (ZallData_APP_JS_Bridge.zalldata_verify) {
            if (!ZallData_APP_JS_Bridge.zalldata_verify(JSON.stringify(_.extend({
                server_url: sd.para.server_url
              }, originData)))) {
              if (sd.para.app_js_bridge.is_send) {
                sd.debug.apph5({
            ...
            ZallData_APP_JS_Bridge.zalldata_track(JSON.stringify(_.extend({
              server_url: sd.para.server_url
            }, originData)));
            (typeof callback === 'function') && callback();
          }
          ...
          if (/zall-verify/.test(navigator.userAgent)) {
            ...
            if (match && match[0] && (typeof match[1] === 'string') && (match[1].split('?').length === 2)) {
              ...
              if (hostname && hostname === match[0] && project && project === match[1]) {
                ...
                iframe.setAttribute('src', 'zallanalytics://trackEvent?event=' + encodeURIComponent(JSON.stringify(_.extend({
                  server_url: sd.para.server_url
                }, originData))));
    ...
    var heatmap = sd.heatmap = {
      ...
      initHeatmap: function() {
        ...
        if (sd.para.heatmap.collect_elements === 'all') {
          ...
          _.addEvent(document, 'click', function(e) {
            ...
            if (tagName === 'a' || tagName === 'button' || tagName === 'input' || tagName === 'textarea' || _.hasAttribute(target, 'data-zall-click')) {
              that.start(ev, target, tagName);
      ...
      initDefineMode : function(){
        function getAndPostDebugInfo(){
            var arr = [];
            if(!((window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.zalldataNativeTracker && _.isObject(window.ZallData_iOS_JS_Bridge) && window.ZallData_iOS_JS_Bridge.zalldata_app_server_url)
            ||(_.isObject(window.ZallData_APP_New_H5_Bridge) && window.ZallData_APP_New_H5_Bridge.zalldata_get_server_url && window.ZallData_APP_New_H5_Bridge.zalldata_track))){
            ...
            if(ZallData_App_Visual_Bridge && ZallData_App_Visual_Bridge.zalldata_visualized_alert_info){
              ZallData_App_Visual_Bridge.zalldata_visualized_alert_info(JSON.stringify(data));
            }else if(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.zalldataNativeTracker && window.webkit.messageHandlers.zalldataNativeTracker.postMessage){
              window.webkit.messageHandlers.zalldataNativeTracker.postMessage(JSON.stringify(data));
            }
    
        }
        if(_.isObject(window.ZallData_App_Visual_Bridge) && window.ZallData_App_Visual_Bridge.zalldata_visualized_mode && ((window.ZallData_App_Visual_Bridge.zalldata_visualized_mode === true) || (window.ZallData_App_Visual_Bridge.zalldata_visualized_mode()))){
          if(_.isObject(sd.para.heatmap) && sd.para.heatmap.clickmap == 'default'){
