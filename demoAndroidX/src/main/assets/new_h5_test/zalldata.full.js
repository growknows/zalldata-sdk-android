  ...
  if(sd.para.use_app_track_config.use_app_track === true || sd.para.use_app_track_config.use_app_track === 'only'){
    ...
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.zalldataNativeTracker && _.isObject(window.ZallData_iOS_JS_Bridge) && window.ZallData_iOS_JS_Bridge.zalldata_app_server_url) {
      if(sd.para.use_app_track_config.H5verify){
          ...
          window.webkit.messageHandlers.zalldataNativeTracker.postMessage(JSON.stringify({callType:'app_h5_track', data: _.extend({server_url:sd.para.server_url}, originData)}));
      }else{
        sd.debug.apph5({
          data: originData,
          step: '4.1',
      ...
      if(sd.para.use_app_track_config.H5verify){
        ...
        ZallData_APP_New_H5_Bridge.zalldata_track(JSON.stringify(_.extend({server_url:sd.para.server_url},originData)));
      }else{
        sd.debug.apph5({
          data: originData,
          step: '4.2',
