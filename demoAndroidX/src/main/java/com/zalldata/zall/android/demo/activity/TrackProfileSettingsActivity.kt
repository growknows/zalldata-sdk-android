...
package com.zalldata.zall.android.demo.activity
...
class TrackProfileSettingsActivity : BaseActivity() {
    ...
    fun doOnClick(view: View) {
        when (view.id) {
            //产生一个自定义事件
            R.id.track_a_event -> {
                ZallDataAPI.sharedInstance(this).track("ViewProduct",
            ...
            R.id.track_installation -> {
                ...
                ZallDataAPI.sharedInstance(this).trackAppInstall(PropertyBuilder.newInstance().append("pKey", "pValue").toJSONObject())
            }
            //匿名 ID 和用户 ID 关联
            R.id.login_btn -> {
                ZallDataAPI.sharedInstance(this).login("130xxxx1234",
                        PropertyBuilder.newInstance()
                                .append("grade", 4)
            ...
            R.id.profile_set_btn -> {
                ZallDataAPI.sharedInstance(this).profileSet(PropertyBuilder.newInstance()
            ...
            R.id.profile_increment_btn -> {
                ZallDataAPI.sharedInstance(this).profileIncrement("money", 2)
            }
            R.id.profile_append -> {
                ZallDataAPI.sharedInstance(this).profileAppend("likesport", "sport" + Random().nextInt(100))
            }
            R.id.profile_delete -> {
                ZallDataAPI.sharedInstance().profileDelete()
            }
            R.id.item_set -> {
                ZallDataAPI.sharedInstance().itemSet("itemType", "itemId", PropertyBuilder
                        .newInstance().append("item", "item").toJSONObject())
            }
            R.id.item_delete -> {
                ZallDataAPI.sharedInstance().itemDelete("itemType", "itemId")
            }
            R.id.trackChannelEvent -> {
                ZallDataAPI.sharedInstance().trackChannelEvent("hello_world211")
            }
            R.id.track_view_onclick -> {
                ZallDataAPI.sharedInstance().trackViewAppClick(track_view_onclick)
            }
            R.id.track_view_screen -> {
                ZallDataAPI.sharedInstance().trackViewScreen(this)
            }
            else -> {
                //no op
            }
